...
const MAX_VALUE_FIELDS = 32;
const MAX_VALUE_KEY_LENGTH = 64;
const reviewRateBuckets = new Map<string, { count: number; resetAt: number; inFlight: number }>();
...
router.post("/review-calculation", async (req, res) => {
  ...
  if (!bucket.allowed) {
    ...
    return res.status(429).json({ error: "Too many calculation reviews. Please try again later." });
  }
  if (bucket.inFlight >= MAX_IN_FLIGHT_PER_CLIENT) {
    return res.status(429).json({ error: "Too many calculation reviews in progress." });
  }
  bucket.inFlight += 1;
...
function isNumberRecord(value: unknown): value is Record<string, number> {
  ...
  return entries.length > 0 && entries.length <= MAX_VALUE_FIELDS && entries.every(([name, item]) => name.length <= MAX_VALUE_KEY_LENGTH && typeof item === "number" && Number.isFinite(item));
}

function consumeRateLimit(clientKey: string) {
  const now = Date.now();
  const current = reviewRateBuckets.get(clientKey);
  const bucket = !current || current.resetAt <= now
    ? { count: 0, resetAt: now + RATE_LIMIT_WINDOW_MS, inFlight: 0 }
    : current;
  bucket.count += 1;
  reviewRateBuckets.set(clientKey, bucket);
  if (reviewRateBuckets.size > 10_000) {
    for (const [key, value] of reviewRateBuckets) {
