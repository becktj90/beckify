...
export function HexGL() {
  ...
  return (
    <section className="space-y-6" aria-labelledby="hexgl-title">
      ...
      <div
        ref={stageRef}
        className={`game-stage relative mx-auto overflow-hidden bg-black shadow-[0_20px_60px_rgba(0,0,0,.35)] ${immersive ? "fixed inset-0 z-[70] rounded-none border-0" : "aspect-video min-h-[480px] max-w-[960px] rounded-2xl border border-[#2e5d86]"}`}
      >
        <iframe
          src={HEXGL_SRC}
