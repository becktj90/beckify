...
export function TootTroopers() {
  ...
  useEffect(() => {
    ...
    const draw = (now: number) => { const dt = Math.min(.033, (now - last) / 1000); last = now; const sky = ctx.createLinearGradient(0, 0, 0, H); sky.addColorStop(0, "#16184f"); sky.addColorStop(1, "#49255f"); ctx.fillStyle = sky; ctx.fillRect(0, 0, W, H); ctx.fillStyle = "rgba(255,255,255,.5)"; for (let i = 0; i < 34; i++) ctx.fillRect((i * 83 + elapsed * 14) % W, (i * 47) % H, 2, 2);
      if (statusRef.current === "running") { elapsed += dt; velocity += 17 * dt; y += velocity; if (gates[gates.length - 1].x < W - 130) gates.push({ x: W + 60, gapY: 150 + Math.random() * 320, scored: false }); gates.forEach(g => { g.x -= 135 * dt; if (!g.scored && g.x + 54 < 128) { g.scored = true; scoreValue++; if (now - lastHud > 150) { setScore(scoreValue); lastHud = now; } tone(720, .1, .025); } }); gates = gates.filter(g => g.x > -80); for (const g of gates) if (128 + 22 > g.x && 128 - 22 < g.x + 54 && (y - 17 < g.gapY - 76 || y + 17 > g.gapY + 76)) crash(); if (y < 16 || y > H - 16) crash(); }
      gates.forEach(g => { ctx.fillStyle = "#ef6e66"; ctx.fillRect(g.x, 0, 54, g.gapY - 76); ctx.fillRect(g.x, g.gapY + 76, 54, H); ctx.fillStyle = "#ffcc75"; ctx.fillRect(g.x - 7, g.gapY - 84, 68, 12); ctx.fillRect(g.x - 7, g.gapY + 72, 68, 12); }); drawRider(); ctx.fillStyle = "#fff"; ctx.font = "700 18px sans-serif"; ctx.fillText(`TOOTS ${scoreValue}`, 18, 34); ctx.fillStyle = "#d8ddff"; ctx.font = "12px monospace"; ctx.fillText(`${riderRef.current.toUpperCase()} // BEST ${best}`, 18, 54); raf = requestAnimationFrame(draw); };
