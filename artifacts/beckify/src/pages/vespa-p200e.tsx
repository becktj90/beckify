...
import { SchemaHead, breadcrumbListSchema, techArticleSchema, toCanonicalUrl } from "@/components/seo/SchemaHead";
import { PAGE_SEO, VESPA_RELATED_TOOLS, toolboxPermalink } from "@/data/seo-copy.mjs";
import {
  HubMotorVisual,
  PowerElectronicsMap,
...
export default function VespaP200EPage() {
  return (
    <Layout className="vespa-journal" showAds={false}>
      ...
      {/* Layout already provides the page's <main> landmark; a nested one
          would be invalid HTML, so this is a plain div kept only for the
          #story anchor the journal nav links to. */}
      <div id="story">
        <FadeIn>
        ...
        <VespaPartsCatalog />
        <FadeIn>
          <section className="journal-section" id="related-calculators" aria-labelledby="related-calculators-heading">
            <p className="section-index">Calculators used on this build</p>
            <h2 id="related-calculators-heading" className="section-heading" style={{ fontFamily: "Georgia, serif", fontSize: "clamp(2.35rem,5vw,4.5rem)", lineHeight: 0.95, letterSpacing: "-0.045em", margin: 0 }}>
              Related electrical calculators.
            </h2>
            <p className="mt-4 max-w-2xl" style={{ color: "var(--muted)" }}>
              Pack layout, conductor sizing, voltage drop, and motor current for the 72V conversion. These are the same toolbox pages linked from the rest of the site — not a second set of doorway pages.
            </p>
            <div className="cta-links" style={{ marginTop: "1.5rem" }}>
              {VESPA_RELATED_TOOLS.map(({ slug, label }) => (
                <a key={slug} href={toolboxPermalink(slug)}>
                  {label}
                </a>
              ))}
            </div>
