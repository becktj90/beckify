...
struct LookCheckPhotoView: View {
    ...
    private func analyze() async {
        ...
        guard let prepared = BeckifyVisionClient.prepareUpload(from: preview) else {
...
/// Look Check wrapper around `BeckifyVisionClient`. Encoding happens only here
/// after the user taps Analyze Look.
enum PhotoLookCheckClient {
    static func analyze(
        ...
        let payload = try await BeckifyVisionClient.postJSON(url: url, body: body, token: token, task: .look)
