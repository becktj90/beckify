...
| **Broken or bait OCR** | Category risk (not a single app): garbage fields saved as truth. | Confirm marks reviewed. MOCP/LRA ≠ FLA. On-device Vision first. Cloud Analyze is optional / user-initiated only. | Screenshot must show the confirm step, not a magic “done” plate. |
| **Permission / privacy surprise** | Utility-app Review rejections: missing usage strings; location at launch; nutrition label vs an SDK. | Usage strings in the target; location/mic/BT/Local Network/camera only in-tool; `PrivacyInfo.xcprivacy` declares Photos for user-initiated Analyze (Look Check, nameplate, panel — not linked, not tracking). | Connect nutrition label = **Photos** (App Functionality). Review notes must say Analyze / Analyze Look is user-initiated; Vision still works offline. |
...
- [ ] Do not bump marketing version just to spam uploads. **1.0 is approved and closed** — the next train must be **1.0.1** / `CURRENT_PROJECT_VERSION` **≥124**. Do not retry **1.0 (121)**.
