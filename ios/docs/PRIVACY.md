...
Apple’s App Privacy nutrition label for this app is **Photos** (App Functionality) when you use Look Check **Analyze Look**. That upload is user-initiated, not linked to an account, and not used for tracking. Sensor readings and Saved Jobs stay on the device. Motor Nameplate OCR and Panel Directory photos also stay on the device.
...
| Camera | Motor Nameplate OCR; Panel Directory; Look Check | Motor nameplate or panel schedule: a still photo for on-device Vision. Look Check: a still photo you may later upload with **Analyze Look**. Taking or choosing a photo does not upload it. Nameplate and panel photos are not uploaded and are not kept as images in Saved Jobs. |

Photo Library full access is **not** requested. Look Check, Panel Directory, and Motor Nameplate OCR can use the system photo picker (`PhotosPicker`) so you choose one or more images. Motor Nameplate OCR, Panel Directory, and Look Check can also use the camera for a still photo. Nameplate and panel photos stay on this device. A Look Check photo leaves the device only after you tap **Analyze Look**.
