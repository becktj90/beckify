...
| Motion & Fitness | Barometer; Bubble Level / plumb; Magnetometer; g-Force Snapshot; Solar Design Wizard (optional panel aim) | On-device `CMAltimeter` pressure / relative altitude and `CMMotionManager` gravity, heading, and acceleration. Requested when you open those tools, not at launch. Not uploaded. Design aid only — not a calibrated instrument. If the barometer is missing or Motion & Fitness is off, Barometer shows an unavailable state instead of crashing. |
...
CoreMotion tools use the Motion & Fitness (`NSMotionUsageDescription`) string on the generated Info.plist. Battery and thermal state are local diagnostics and do not use that key.
