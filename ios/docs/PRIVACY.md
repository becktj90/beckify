...
| Camera | Motor Nameplate OCR; Look Check | Motor nameplate: a still photo for on-device Vision. Look Check: a still photo you may later upload with **Analyze Look**. Taking or choosing a photo does not upload it. Nameplate photos are not uploaded and are not kept in Saved Jobs. |

Photo Library full access is **not** requested. Look Check, Panel Directory, and Motor Nameplate OCR can use the system photo picker (`PhotosPicker`) so you choose one image. Motor Nameplate OCR and Look Check can also use the camera for a still photo. Nameplate and panel photos stay on this device. A Look Check photo leaves the device only after you tap **Analyze Look**.
