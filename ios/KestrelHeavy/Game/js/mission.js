...
import { applyHit, applyScrape, healthKind, healthMaxFor } from './health.js';
import {
  BOOSTER_HIT,
  ROCKET_HIT,
  contactsHazard,
...
export default class MissionScene extends Phaser.Scene {
  ...
  create() {
    ...
    installTextures(this, this.currentFlight().tod, this.settings.loadout);
    ...
    this.milky = this.add.image(W / 2, -2400, 'milky').setDepth(-1.8).setDisplaySize(W + 520, 1600).setAlpha(0).setBlendMode('ADD');
    this.nebula = this.add.image(W / 2, -2000, 'nebula').setDepth(-1.7).setDisplaySize(W + 480, 1400).setAlpha(0).setBlendMode('ADD');
    ...
    try {
      this.windFx = this.add.particles(0, 0, 'wind', {
      ...
      this.windFx.setDepth(3);
    } catch {
      this.windFx = noopEmitter;
    }
    this.heat = this.add.image(PAD_ROCKET_X, PAD_ROCKET_Y + 118, 'heat')
      .setDepth(5.5)
      .setBlendMode('ADD')
      .setAlpha(0)
      .setVisible(true);
    this.windGfx = this.add.graphics().setDepth(3.2);
    ...
    this.input.on('pointerdown', (pointer) => {
      ...
      else if (this.status === 'PRELAUNCH') this.requestLaunch();
  ...
  freshSession() {
    ...
    return {
      ...
      throttle: 0,
      shield: 0,
      health: healthMaxFor(this.settings.difficulty),
      overdrive: 0,
  ...
  playfieldSize() {
    ...
    const w = Math.round(parent?.width || this.scale?.width || W);
    const h = Math.round(parent?.height || this.scale?.height || H);
  ...
  syncFollowOffset() {
    ...
    if (this.status === 'JACKLYN' && this.rocket && this.jacklyn) {
      const { w: pw, h: ph } = this.playfieldSize();
      const vis = envelopVisibleWorld(pw, ph, W, H, cam.zoom || 1);
      const off = havenFollowOffset(
        this.rocket.x,
        this.rocket.y,
        this.jacklyn.x,
        this.jacklyn.y,
      ...
      cam.setFollowOffset(off.x, off.y);
      return;
    }
  ...
  requestLaunch() {
    if (!this.session || this.paused || this.settingsOpen) return;
    if (this.status !== 'PRELAUNCH') return;
    if (this.session.launchArmed) {
      flashIgnite();
      this.flashPad(0.22);
      return;
    }
  ...
  flightZoom(kind) {
    ...
    if (kind === 'pad') return padZoomForPlayfield(w, h, false);
    return ascentZoomForPlayfield(w, h, this.session?.altitudeKm || 0, false);
  }
  ...
  startMission() {
    ...
    const padZoom = this.flightZoom('pad');
    this.cameras.main.setZoom(padZoom);
    this.cameras.main.centerOn(W / 2, H / 2);
    this.setZoomWant(padZoom, CAM.zoomClimbRate);
    this.paintSkyWash(0);
  ...
  liftoff() {
    ...
    this.setZoomWant(this.flightZoom('pad'), CAM.zoomClimbRate);
  ...
  enterJacklyn() {
    ...
    this.session.jacklynReadyAt = this.nowSec + HAVEN.readySec;
    ...
    this.rocket.setIgnoreGravity(false);
    setBoostHeld(this.inputState, false, this.nowSec);
    this.inputState.boostHeld = false;
    this.matter.world.setGravity(0, HAVEN.reentryGravity);
    this.cameras.main.centerOn(this.rocket.x, this.rocket.y);
    this.lockVehicleCamera(CAM.havenLerpX, CAM.havenLerpY);
    const { w, h } = this.playfieldSize();
    const zoom = havenZoomWant({
      alt: this.jacklyn.y - this.rocket.y,
      pulling: false,
    ...
    this.time.delayedCall(Math.round((HAVEN.reentrySec + HAVEN.strakeSec + 1.2) * 1000), () => {
  ...
  updatePrelaunch(dt) {
    ...
    if (launched && (this.session.fired.ignition || this.session.charge >= 0.42)) {
      this.emitPlume(0.85);
      this.flashPad(0.28);
    }
    if (!launched) {
  ...
  updateAscent(dt) {
    ...
    const cruise = isAscentCruise(this.status) && this.session.fuel > 0;
    ...
    if (cruise) {
      ...
      this.session.throttle = clamp(this.session.throttle + dt * 3.2, 0.55, 1);
    } else {
    ...
    const space = spaceBlend(alt, this.status);
    ...
    if (this.status !== 'ASCENT') return;
    this.emitWind(mode.wind || 0);

    this.bgPad.setVisible(this.rocket.y > 80);
    const { w: pw, h: ph } = this.playfieldSize();
    const climbZoom = ascentZoomForPlayfield(pw, ph, this.session.altitudeKm, this.settings.reducedMotion);
  ...
  updateJacklyn(dt) {
    ...
    const assist = this.isHavenQa() ? Math.max(mode.assist, 0.28) : mode.assist;
    ...
    const burnWindow = alt < HAVEN.burnAlt && this.session.jacklynElapsed >= HAVEN.glideMinSec;
    ...
    const burning = this.session.jacklynPhase === 'burn'
      || this.session.jacklynPhase === 'straighten'
      || this.session.jacklynPhase === 'settle';
    if (assist > 0) {
      const err = (deckX - this.rocket.x) / 420;
      const mix = burning ? (mode.assist > 0.55 ? 0.3 : assist * 0.18) : assist * 0.16;
      axis = clamp(axis + err * mix, -1, 1);
    }
    ...
    const wantAngle = reentry
      ? side * -46
      : gliding
        ? clamp(side * -30 + this.rocket.body.velocity.x * 1.4 + axis * 6, -40, 40)
        : clamp(-axis * 18 + this.rocket.body.velocity.x * 1.1, -24, 24);
    const nowAngle = this.rocket.angle || 0;
    const slew = reentry ? 2.2 : gliding ? 1.35 : 3.1;
    ...
    if (reentry) {
      ...
      this.rocket.applyForce({ x: axis * 0.016, y: -0.032 });
      this.session.throttle = 1;
      this.emitPlume(0.9);
      ...
      this.rocket.applyForce({ x: axis * 0.007 + (mode.wind || 0) * 0.0022, y: 0.01 });
    ...
    this.rocket.x = clamp(this.rocket.x, -1900, W + 1900);
  ...
  hitHazard(obj, label) {
    ...
    this.session.shield = result.shield;
    obj.destroy();
    ...
    if (result.kind === 'kid') {
      this.session.combo = 0;
      AudioApi.play('hit', this.settings);
      this.callout('close-call', {
        banner: `GRAZE — INT ${Math.round(result.health)} · KID keeps you flying`,
        kind: 'warn',
        holdMs: 900,
      });
      return;
    ...
    AudioApi.play('hit', this.settings);
    if (result.kind === 'lethal' && mode.allowFail) {
      this.callout('health-lethal', { banner: 'INT 0 — STRUCTURAL LIMIT', kind: 'fail', holdMs: 900 });
      this.failFlight(hazardAbortId(kind, atMaxQ));
      return;
    }
    this.session.combo = 0;
    const pct = Math.round(result.health);
    this.callout('health-graze', {
      banner: `GRAZE — INT ${pct}  ·  not lethal`,
      kind: 'warn',
      holdMs: 1400,
    });
  }
  ...
  collectPickup(obj) {
    ...
    if (kind === 'shield') {
      ...
      const unlocked = grantPickupUnlock(this.settings, 'shield');
      if (unlocked) {
        mergeUnlocks(this.settings);
        saveSettings(this.settings);
        this.refreshHangar();
        setBanner(`HANGAR — ${unlocked.id} unlocked`, 'go', 1400);
      }
    } else if (kind === 'fuel') {
      ...
      const unlocked = grantPickupUnlock(this.settings, 'fuel');
      if (unlocked) {
        mergeUnlocks(this.settings);
        saveSettings(this.settings);
        this.refreshHangar();
      ...
      const unlocked = grantPickupUnlock(this.settings, 'boost');
      if (unlocked) {
        mergeUnlocks(this.settings);
        saveSettings(this.settings);
        this.refreshHangar();
  ...
  flashPad(alpha) {
    ...
    this.padGlow.clear();
    this.padGlow.fillStyle(missionTod(this.currentFlight()).padGlow, alpha);
  ...
  emitPlume(power) {
    ...
    const twr = this.session?.science?.twr || power;
    const strength = clamp(Math.max(power, twr * 0.28), 0, 1.6);
    const n = strength > 0.7 ? (space ? 8 + Math.round(strength * 6) : 10 + Math.round(strength * 8)) : 3 + Math.round(strength * 4);
    this.plume.emitParticleAt(this.rocket.x, this.rocket.y + 108, n);
    if (this.heat) {
      this.heat.setPosition(this.rocket.x, this.rocket.y + 112);
      this.heat.setAlpha(0.25 + strength * 0.55);
      this.heat.setScale(0.7 + strength * 0.9, 0.9 + strength * 1.15);
      this.heat.setAngle((this.nowSec * 40) % 8 - 4);
    }
  ...
  emitWind(wind, gust) {
    if (!this.rocket) return;
    const power = Number(wind) || 0;
    if (this.windGfx) this.windGfx.clear();
    if (this.settings.reducedMotion || power < 0.08) return;
    const g = gust == null ? Math.sin(this.nowSec * 0.9) * power : gust;
    const dir = g >= 0 ? 1 : -1;
    if (this.status === 'ASCENT') {
      const lean = clamp(this.rocket.angle + dir * power * 4, -18, 18);
      this.rocket.setAngle(lean);
      this.rocket.applyForce({ x: g * 0.0012, y: 0 });
    }
    if (this.windFx) {
      if (this.windFx.speedX !== undefined) {
        this.windFx.speedX = dir >= 0 ? { min: 40, max: 120 } : { min: -120, max: -40 };
      }
      const n = this.isCoarsePointer() ? 1 : 2;
      this.windFx.emitParticleAt(this.rocket.x - dir * 78, this.rocket.y - 10, n);
      this.windFx.emitParticleAt(this.rocket.x - dir * 52, this.rocket.y + 36, 1);
    }
    if (this.windGfx) {
      this.windGfx.lineStyle(2, 0xc8e8ff, 0.22 + power * 0.4);
      for (let i = 0; i < 4; i++) {
        const y = this.rocket.y - 70 + i * 38;
        const x0 = this.rocket.x - dir * (86 + i * 10);
        this.windGfx.lineBetween(x0, y, x0 + dir * (46 + power * 36), y + 3);
      }
    }
  }
  ...
  updateFlightFx(dt) {
    if (this.heat) {
      const thrusting = (this.session?.throttle || 0) > 0.08 && this.inFlight();
      if (!thrusting || this.settings.reducedMotion) {
        this.heat.setAlpha(Math.max(0, (this.heat.alpha || 0) - dt * 3.2));
      } else {
        this.heat.setPosition(this.rocket.x, this.rocket.y + 112);
      }
    }
    if (this.windGfx && (this.status === 'MENU' || this.status === 'SUMMARY' || this.status === 'SEP')) {
      this.windGfx.clear();
    }
  }
  ...
  endMission(reason) {
    ...
    clearFlightHolds(this.inputState);
    if (this.session) this.session.launchArmed = false;
    this.cameras.main.stopFollow();
    this.refreshHud();
  }
  ...
  enforceCorridor(dt) {
    ...
    const scrape = applyScrape(this.session, dt, this.settings.difficulty);
    this.session.health = scrape.health;
    this.callout('health-scrape', { banner: 'CORRIDOR SCRAPE — steer back in', kind: 'fail', holdMs: 1200 });
    if (scrape.kind === 'lethal') {
  ...
  paintSkyWash(spaceT) {
    ...
    const wash = missionTod(this.currentFlight()).wash;
    const mid = lerpRgb(wash.mid, wash.space, t);
    const near = lerpRgb(wash.horizon, wash.space, t * 0.92);
    const lowA = Math.max(0, 0.92 - t * 0.92);
    this.bgWash.clear();
    this.bgWash.fillStyle(wash.space, 0.35 + t * 0.65);
    this.bgWash.fillRect(-400, CAM.worldTop, W + 800, CAM.worldHeight * 0.62);
    this.bgWash.fillStyle(mid, 0.55 + t * 0.45);
    this.bgWash.fillRect(-400, -1800, W + 800, 1600);
    this.bgWash.fillStyle(near, 1);
    this.bgWash.fillRect(-400, -220, W + 800, 520);
    this.bgWash.fillStyle(wash.low, lowA);
  ...
  showAscentSky(on) {
    ...
    if (this.milky) this.milky.setVisible(on);
    if (this.nebula) this.nebula.setVisible(on);
  ...
  updateSkyLayers(dt = 0.016) {
    if (!this.session || this.status === 'JACKLYN' || this.status === 'SUMMARY') return;
    const target = spaceBlend(this.session.altitudeKm || 0, this.status);
    const prev = Number.isFinite(this.session.spaceT) ? this.session.spaceT : target;
    const rate = this.status === 'SEP' ? 0.85 : 2.2;
    this.session.spaceT = prev + (target - prev) * Math.min(1, dt * rate);
    const space = this.session.spaceT;
    const layers = skyLayerAlphas(space);
    this.paintSkyWash(space);
    if (this.stars && this.rocket) {
      this.stars.setAlpha(layers.stars);
      this.stars.setPosition(this.rocket.x * 0.04 + W / 2, this.rocket.y * 0.08 - 1800);
      this.stars.setDisplaySize(W + 520, 3200);
    }
    if (this.milky && this.rocket) {
      this.milky.setAlpha(layers.milky);
      this.milky.setPosition(this.rocket.x * 0.03 + W / 2, this.rocket.y * 0.05 - 2100);
    }
    if (this.nebula && this.rocket) {
      this.nebula.setAlpha(layers.nebula);
      this.nebula.setPosition(this.rocket.x * 0.05 + W / 2, this.rocket.y * 0.07 - 1900);
    }
    if (this.bgSky && this.rocket) {
      this.bgSky.y = -1480 + this.rocket.y * 0.06;
      this.bgSky.setAlpha(layers.sky);
    }
    if (this.cloudsFar && this.rocket) {
      this.cloudsFar.y = 80 + this.rocket.y * 0.16;
      this.cloudsFar.x = W / 2 + Math.sin((this.nowSec || 0) * 0.08) * 18;
      this.cloudsFar.setAlpha(layers.clouds * 0.72);
    }
    if (this.cloudsNear && this.rocket) {
      ...
      this.cloudsNear.setAlpha(layers.clouds * 0.58);
    }
    if (this.hazeBand && this.rocket) {
      this.hazeBand.y = 340 + this.rocket.y * 0.2;
      this.hazeBand.setAlpha(layers.haze * 0.8);
    }
  }
  ...
  closeSettings() {
    ...
    if (this.pausedForSettings) {
      ...
      const sheet = this.screen === 'missions'
        ? 'ng-missions'
        : this.screen === 'howto'
          ? 'ng-howto'
          : this.screen === 'hangar'
  ...
  openScreen(name) {
    ...
    else if (name === 'hangar') showScreen('ng-hangar');
  ...
  openHangar() {
    mergeUnlocks(this.settings);
    this.refreshHangar();
    this.openScreen('hangar');
  }

  refreshHangar() {
    setHangar(this.settings, { LOADOUT_CATS }, comboCount(), (cat, id) => this.equipPart(cat, id));
  }

  equipPart(cat, id) {
    mergeUnlocks(this.settings);
    if (!isPartUnlocked(this.settings.unlockedParts, cat, id)) {
      setBanner('LOCKED — fly KH-n, score, or grab pickups to unlock', 'warn', 1800);
      return;
    }
    this.settings.loadout = { ...this.settings.loadout, [cat]: id };
    saveSettings(this.settings);
    this.applyLook();
    this.refreshHangar();
    AudioApi.play('ui', this.settings);
  ...
  applyLook() {
    const flight = this.currentFlight();
    refreshTodTextures(this, flight.tod);
    refreshLoadoutTextures(this, this.settings.loadout, flight);
    if (this.bgPad) this.bgPad.setTexture('pad');
    if (this.bgSky) this.bgSky.setTexture('ascent-sky');
    if (this.cloudsFar) this.cloudsFar.setTexture('clouds-far');
    if (this.cloudsNear) this.cloudsNear.setTexture('clouds-near');
    if (this.hazeBand) this.hazeBand.setTexture('haze');
    if (this.bgOcean) this.bgOcean.setTexture('ocean');
    this.applyRocketSkin();
    this.paintSkyWash(this.status === 'SEP' ? 1 : 0);
  }

  applyRocketSkin() {
    const key = this.textures.exists('rocket-equipped') ? 'rocket-equipped' : 'rocket';
    if (this.status === 'JACKLYN') return;
    this.rocket.setTexture(key);
    this.bindRocketBody();
  }
  ...
  refreshHud() {
    ...
    renderHud({
      ...
      health: `${Math.round(s.health == null ? healthMaxFor(this.settings.difficulty) : s.health)}`,
      healthText: `${Math.round(s.health == null ? healthMaxFor(this.settings.difficulty) : s.health)}`,
      healthMax: healthMaxFor(this.settings.difficulty),
      healthKind: healthKind(
        s.health == null ? healthMaxFor(this.settings.difficulty) : s.health,
        healthMaxFor(this.settings.difficulty),
      ),
      hideHealth: this.status === 'MENU' || this.status === 'SUMMARY',
      ...
      boostLabel: this.status === 'JACKLYN'
        ? ((s.burnWindow || s.jacklynPhase === 'burn' || s.jacklynPhase === 'straighten' || s.jacklynPhase === 'settle')
          ? 'HOLD TO BURN'
          : 'GLIDE')
    ...
    document.body.dataset.tod = flight.tod || '';
    this.syncCanvasInput();
