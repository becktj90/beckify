...
**Analyze** POSTs an upright JPEG to `https://api.beckify.com/api/analyze-look` with `roastMode` `mean` or `nice`. The Beckify API may forward that photo to OpenAI and/or Anthropic. The result is an entertainment verdict plus a comedy roast when the subject appears 18+ and is rated.
